...
// fill array with small random var
static void toy_fill_small(short *buf, int n) {
    for (int k = 0; k < n; ++k) {
        // small Binomial distribution using Hamming weight
        short val = rand() & 0xF;
        val = (val >> 1 & 1) - (val & 1); 
        if (val < 0)
            val += TK_Q;
...
// perform polynomial multiplication naively
static void toy_polmul_naive(short *dst, const short *a, const short *b, int add) {
    // Naive polynomial multiplication for elements in A and t matrices
...
// perform matrix-vector multipication
...
// perform matrix transpose times vector multiplication
...
// add 2 vectors with optional negation of the second
...
// generate random elements for toy encryption
...
// decrypt ciphertext and check against secret key
